...
def analyze(symbol):
    try:
        ...
        if df is None or len(df) < 10:
            return f"⚠️ {symbol}: لا توجد بيانات"
        ...
        orb_high = round(df.head(3)["High"].max(), 2)
        orb_low = round(df.head(3)["Low"].min(), 2)

        call_conditions = (price > vwap and price > orb_high and ema9 > ema21 and rsi < 70)
        put_conditions = (price < vwap and price < orb_low and ema9 < ema21 and rsi > 30)

        if call_conditions:
            signal = "CALL 📈"
            reason = "فوق VWAP + كسر ORB + EMA صاعد"
            stop = round(orb_high - (orb_high - orb_low) * 0.5, 2)
            target = round(price + (price - stop) * 2, 2)
            trade_info = f"\n✅ دخول: <b>{price}</b>\n🛑 وقف: <b>{stop}</b>\n🎯 هدف: <b>{target}</b>"
        elif put_conditions:
            signal = "PUT 📉"
            reason = "تحت VWAP + كسر ORB + EMA هابط"
...
def analyze_all():
    ...
    for symbol in ASSETS:
        msgs.append(analyze(symbol))
        time.sleep(15)  # Alpha Vantage حد 5 طلبات في الدقيقة
    return "\n".join(msgs)

def main():
    global offset
    print("✅ البوت شغال - Alpha Vantage + ORB + VWAP")
    last_auto_run = -1
            ...
            for update in updates.get("result", []):
                ...
                if "message" in update:
                    ...
                    if text == "/start":
                        ...
                        send_message(chat_id, "✅ تم تفعيل الإشارات كل 10 دقائق!")
                    elif text == "/stop":
