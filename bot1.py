...
def analyze(symbol, ticker_code):
    try:
        ...
        df = ticker.history(period="1d", interval="10m")

        if df.empty or len(df) < 5:
            return f"⚠️ {symbol}: السوق مغلق أو لا توجد بيانات"
...
def analyze_all():
    ...
    for symbol, code in ASSETS.items():
        msgs.append(analyze(symbol, code))
    return "\n".join(msgs)

def main():
    global offset
    threading.Thread(target=run_server, daemon=True).start()
    print("✅ البوت شغال - yfinance + ORB + VWAP")
    last_auto_run = -1
